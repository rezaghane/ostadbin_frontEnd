	...
	<div class="w-100 p-3 bg-white border-15 h-center">
		<div style="width: 75%; max-width: 500px;">
			...
			<btn
				...
				disableText="عنوان مقطع را وارد کنید"
			/>
		</div>
	</div>
</template>
		...
		export default {
			...
			methods:{
				loader(THIS = this){
					...
					$.post(this.baseurl+"degrees/" + this.$route.params.id, { token: this.$localStorage.get("user") }, function(D){
				...
				sav(THIS = this){
					...
					THIS.loading(THIS);
					$.post(this.baseurl+'degrees/' + mod, {
						role: this.$localStorage.get('user').username.role,
						name: THIS.val.name,
						status: THIS.val.status, token: this.$localStorage.get("user")
					}, function(D){
						THIS.success(THIS);
						THIS.$router.push('/degrees/list');
					}, "json");
				},
			}
		}
