<template>
	...
	<script>
		...
		export default {
			...
			mounted(){
				(this.$route.params.id != undefined) ? this.loader() : this.getDegrees();
			},
			methods:{
				getDegrees(p=1, search='',THIS = this){
					$.post(this.baseurl+"degrees/list?search="+search+"&p="+p, { role: 1, token: this.$localStorage.get("user") }, function(D){
						THIS.degrees = D;
					}, "json");
				},
				...
				loader(THIS = this){
					...
					$.post(this.baseurl+"category/" + this.$route.params.id, {token: this.$localStorage.get("user") }, function(D){
						if(D.status.code=="401"){
							THIS.$localStorage.remove('user');
							THIS.$router.push('/login');
						}else{
							THIS.val.degrees = D.data.degrees;
				...
				sav(THIS = this){
					...
					THIS.loading(THIS);
					$.post(this.baseurl+'category/' + mod, {
						role: this.$localStorage.get('user').username.role,
						name: THIS.val.name,
						status: THIS.val.status,
						degrees: THIS.val.degrees, token: this.$localStorage.get("user")
					}, function(D){
						THIS.success(THIS);
