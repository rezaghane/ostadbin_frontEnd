		...
		>
			<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 float-left p-3">
				<back
					...
					showBtn="false"
					leftBtn="true"
					leftBtnText="+ افزودن دسته بندی جدید"
					leftBtnType="success"
					@clickMe="rtrNewClass()"
				/>
				<input
	...
	<script>
		...
		export default {
			...
			methods:{
				...
				rtrNewClass(){
					this.$router.push('/degrees/create');
				},
				// *********************************************************************
				loader(p=1, search='',THIS = this){
					document.body.scrollTop = 0;
					THIS.flds = {ok: null, data: []};
					$.post(this.baseurl+"degrees/list?search="+search+"&p="+p, { role: this.$localStorage.get("user").username.role, token: this.$localStorage.get("user") }, function(D){
