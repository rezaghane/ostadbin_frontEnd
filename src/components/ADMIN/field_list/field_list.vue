		...
		>
			<div class="col-lg-9 col-md-9 col-sm-12 col-xs-12 float-left p-3">
				<back
					...
					showBtn="false"
					leftBtn="true"
					leftBtnText="+ افزودن رشته تحصیلی جدید"
					leftBtnType="success"
					@clickMe="rtrNewClass()"
				/>
				<input
	...
	<script>
		...
		export default {
			...
			methods:{
				...
				rtrNewClass(){
					this.$router.push('/field/create');
				},
				// *********************************************************************
				loader(p=1, search='',THIS = this){
					document.body.scrollTop = 0;
					this.flds = {ok: null, data: []};
					$.post(this.baseurl+"field/list?search="+search+"&p="+p, { token: this.$localStorage.get("user"), role: this.$localStorage.get("user").username.role }, function(D){
