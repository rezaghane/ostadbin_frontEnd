		...
		this.$swal.close();
			...
			methods:{
				...
				loader(p=1,THIS = this){
					...
					$.post(this.baseurl+'classes/'+this.$route.params.id, { token: this.$localStorage.get("user"), teacher: this.$localStorage.get("user").username.id, role: this.$localStorage.get("user").username.role }, function(D){
